...
#if defined(NDEBUG)
# error "TREASURESL cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x03dd0f4d73d6a314c91ddc1618b43a5cb18b35cef6e3ffa286b66d06154bd448");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // TREASURESL: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "TREASURESL Signed Message:\n";
...
{
    // TREASURESL: IsDust() detection disabled, allows any valid dust to be relayed.
...
const int YEARLY_BLOCKCOUNT = 262800000;
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 500 * COIN;

        if(nHeight == 1)
			nSubsidy = 3366000000 * COIN;
    	    ...
    	    nSubsidy= 442.36 * COIN;
    	else if(nHeight < (5 * YEARLY_BLOCKCOUNT))
            nSubsidy= 424.66 * COIN;
        else if(nHeight < (6 * YEARLY_BLOCKCOUNT))
            nSubsidy= 407.67 * COIN;    
        else if(nHeight < (7 * YEARLY_BLOCKCOUNT))
            nSubsidy= 391.36 * COIN;     
        else if(nHeight < (8 * YEARLY_BLOCKCOUNT))
            nSubsidy= 375.70 * COIN;     
        else if(nHeight < (9 * YEARLY_BLOCKCOUNT))
            nSubsidy= 360.67 * COIN;     
        else if(nHeight < (10 * YEARLY_BLOCKCOUNT))
            nSubsidy= 346.24 * COIN;     
        else if(nHeight < (11 * YEARLY_BLOCKCOUNT))
            nSubsidy= 332.39 * COIN;    
        else if(nHeight < (12 * YEARLY_BLOCKCOUNT))
            nSubsidy= 319.09 * COIN;    
        else if(nHeight < (13 * YEARLY_BLOCKCOUNT))
            nSubsidy= 306.32 * COIN;    
        else if(nHeight < (14 * YEARLY_BLOCKCOUNT))
            nSubsidy= 294.06 * COIN;        
        else if(nHeight < (15 * YEARLY_BLOCKCOUNT))
            nSubsidy= 282.29 * COIN;        
        else if(nHeight < (16 * YEARLY_BLOCKCOUNT))
            nSubsidy= 270.99 * COIN;            
        else if(nHeight < (17 * YEARLY_BLOCKCOUNT))
            nSubsidy= 260.15 * COIN;            
        else if(nHeight < (18 * YEARLY_BLOCKCOUNT))
            nSubsidy= 249.74 * COIN;               
        else if(nHeight < (19 * YEARLY_BLOCKCOUNT))
            nSubsidy= 239.75 * COIN;               
        else if(nHeight < (20 * YEARLY_BLOCKCOUNT))
            nSubsidy= 230.15 * COIN;               
        else if(nHeight < (21 * YEARLY_BLOCKCOUNT))
            nSubsidy=220.94 * COIN;               
        else if(nHeight < (22 * YEARLY_BLOCKCOUNT))
            nSubsidy=212.10 * COIN;               
        else if(nHeight < (23 * YEARLY_BLOCKCOUNT))
            nSubsidy=203.61 * COIN;               
        else if(nHeight < (24 * YEARLY_BLOCKCOUNT))
            nSubsidy=195.46 * COIN;               
        else if(nHeight < (25 * YEARLY_BLOCKCOUNT))
            nSubsidy=187.64 * COIN;               
        else if(nHeight < (26 * YEARLY_BLOCKCOUNT))
            nSubsidy=180.13 * COIN;     
        else if(nHeight < (27 * YEARLY_BLOCKCOUNT))
            nSubsidy=172.92 * COIN;         
        else if(nHeight < (28 * YEARLY_BLOCKCOUNT))
            nSubsidy=166.00 * COIN;             
        else if(nHeight < (29 * YEARLY_BLOCKCOUNT))
            nSubsidy=159.35 * COIN;             
        else if(nHeight < (30 * YEARLY_BLOCKCOUNT))
            nSubsidy=152.97 * COIN;                 
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // TREASURESL: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // TREASURESL: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1510573817, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 22/Sept/2009 Is Best One In World J";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04562e62f2035031adfc57e569b897b42899738f1b30a214f60de5cf584d9af9f9c0c64f1cec81d6a63b933d39e69e3ce4d9bd1a3bea329cf8d6c00fec095a5d64") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1537600288;
        ...
        assert(block.hashMerkleRoot == uint256("0x894e3e497ce89a29abc83a05ee64780d70069e95ba8d5a13643991e64f160607"));
...
unsigned char pchMessageStart[4] = { 0xe3, 0xd2, 0x42, 0xe5 }; // TREASURESL: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TREASURESLMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TREASURESLMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TREASURESLMiner(CWallet *pwallet)
{
    printf("TREASURESLMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("treasureSL-miner");
    ...
    try { loop {
        ...
        printf("Running TREASURESLMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TREASURESLMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TREASURESLMiner, pwallet));
}
